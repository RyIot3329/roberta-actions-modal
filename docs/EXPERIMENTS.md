...
| next | – | flip-aware context dropout (flips never shown name-only), fixed head init across seeds (head_init_seed 1234, soup-friendly), gate primary = operational pair view with name-only judged vs the seed mean | | | | | | baseline re-scored with the pair view by CI (stale: no pairs_sha256) |
